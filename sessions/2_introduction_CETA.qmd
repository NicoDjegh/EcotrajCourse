...
format:
  revealjs:
    ...
    incremental: true
    smaller: true
    logo: resources/img/logo-lemar-big.png
    theme: [default, resources/scss/custom.scss]
editor: source
author: "Nicolas Djeghri, Miquel De Cáceres, Anthony Sturbois"
institute: "LEMAR, EMF-CREAF, VivArmor Nature"
toc: false
toc-depth: 2
...
3. Characterizing CETA trajectories
4. Cyclical shifts
:::
...
# 2. CETA-specific conceptual and geometric elements {background-color="#008080"}
...
Start by defining one cyclical trajectory:
...
#Make cyclical data
x <- sin((times*2*pi)/DurC)+trend*times
y <- cos((times*2*pi)/DurC)
mat <- cbind(x,y)
