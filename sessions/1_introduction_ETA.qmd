...
Trajectory data inputs for ETA are of two kinds: (a) trajectory metadata; (b) a distance matrix $\mathbf{D}$ representing the resemblance between ecological states in space $\Omega_0$.

. . .

**Metadata**

In this session we will primarily use a small data set where three entities have been observed four times. The vectors describing entities and observation times are:
...
For simplicity, we will assume that ecological states can be compared using the Euclidean distance, and that only two variables have been measured:
...
The distance matrix $\mathbf{D}$ is obtained using:
...
In addition to segment lengths, one can also calculate the distance between all states and the initial, which is taken as reference:
